...
from networks.linknet import LinkNet34, rgbm_LinkNet34
...
# ROOT = 'dataset/train/'
# NAME = 'trainlog_rgbm_linkNet34_deeplabv3_pred'  # 保存日志名
...
if dataset_name == "deepglobe":
    ROOT = 'dataset/train/'
    NAME = f'trainlog_rgbm_{model_name}_pred'  # 保存日志名
    imagelist = filter(lambda x: x.find('sat') != -1, os.listdir(ROOT))
    alllist = list(map(lambda x: x[:-8], imagelist))
    train_len = int(len(alllist) * 0.8)
    trainlist = alllist[:train_len]
    ...
    NAME = f'{dataset_name}_trainlog_{model_name}_pred'  # 保存日志名
...
def test():
    ...
    labels = []
    num = 0
    solver = MyFrame(rgbm_LinkNet34, dice_bce_loss, 2e-4)
    solver.load("/opt/data/private/workspace/Road-Extraction-master/weights/mass_trainlog_D-linknet_pred.pt")
    val_dataset = ImageFolder(vallist, val_root, train=False,refine=True,dataset_name=dataset_name)
    ...
    for img, mask, erased_gt in tqdm(iter(val_data_loader)):
        ...
        pred, _ = solver.test_batch(refine=True)
        for i in range(pred.shape[0]):
            ...
            pred_i = pred_i.astype(np.uint8) * 255
            cv2.imwrite(f"./dataset/vision/{vallist[num]}_refine.png",pred_i)
            num = num + 1
...
def train():
    solver = MyFrame(rgbm_LinkNet34, dice_bce_loss, 2e-4)
    solver.load("/opt/data/private/workspace/Road-Extraction-master/weights/trainlog_rgbm_linkNet34_pre.pt")

    dataset = ImageFolder(trainlist, train_root, train=True, refine=True,dataset_name=dataset_name)
    ...
    for epoch in range(1, total_epoch + 1):
        ...
        for img, mask, erased_gt in tqdm(iter(data_loader)):
            solver.set_input(img, mask, erased_gt)
            train_loss = solver.optimize(refine=True)
